...
def _prompt(
    ...
    # Istruzioni iniziali
    intro = [
        f"Sei l'ufficio comunicazione del Comune di {city}. Scrivi in italiano. Il portavoce è {mayor}.",
        f"Tono richiesto: {tone}.",
        f"Regole emoji: Social → {emoji_social}. {emoji_non_social}",
        f"Bolding: {_bolding_rule()}",
        "",
        "Contesto da documenti/articoli (usa solo se rilevante, evita ripetizioni):",
        "",
        source_text,
        "",
        f"Temi aggiuntivi richiesti: {topics or '—'}",
        f"Pubblico principale: {audience}",
        f"Foto allegata: {photo_url or 'no'}",
        "",
        "Produci SOLO le sezioni richieste:",
    ]

    # Specifiche di output per sezione
    outputs_spec: List[str] = []
    if "COMUNICATO_STAMPA" in sections:
        outputs_spec.append(
            "1) >>>COMUNICATO_STAMPA<<< (600-900 parole), titolo, occhiello, corpo, citazione del portavoce. "
            "Vietate emoji; niente grassetto decorativo."
        )
    if "SITO_ISTITUZIONALE" in sections:
        outputs_spec.append(
            "2) >>>SITO_ISTITUZIONALE<<< (400-700 parole), H2/H3, punti elenco operativi se utile. "
            "Linguaggio pratico. Vietate emoji; grassetto solo se strettamente necessario su termini tecnici."
        )
    if "SOCIAL_FB_IG" in sections:
        outputs_spec.append(
            "3) >>>SOCIAL_FB_IG<<< (max 900 caratteri), tono empatico ma sobrio, usa il grassetto per parole chiave. "
            + ("Consenti emoji. " if use_emojis_social else "Non usare emoji. ")
            + ("Aggiungi 2-4 hashtag pertinenti. " if add_hashtags else "")
            + ("Chiudi con invito/CTA (date, link, partecipazione)." if add_call_to_action else "")
        )
    if "SOCIAL_X" in sections:
        outputs_spec.append(
            "4) >>>SOCIAL_X<<< (max 280 caratteri), incisivo e chiaro, possibile 1 grassetto mirato. "
            + ("Consenti emoji. " if use_emojis_social else "Non usare emoji. ")
            + ("Aggiungi 1-2 hashtag se pertinenti." if add_hashtags else "")
        )
...
def generate_outputs(
    ...
    prompt = _prompt(
        ...
        tone=tone,
        use_emojis_social=use_emojis_social,
        targets=targets,
    )
    ...
    return {
        ...
        "social_li": _extract("SOCIAL_LI"),  # non prodotto in questa versione, ma chiave presente per coerenza
